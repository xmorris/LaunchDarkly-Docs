...
Permissions are set to `deny` by default except for the `viewProject` and `createAccessToken` actions. You must explicitly deny those actions for the No access role.
...
Here is an example of a custom role that allows all actions in the first statement, then explicitly denies actions on certain resources:
...
[
  {
    "effect": "allow",
    "actions": ["*"],
    "resources": ["*"] //Error parsing resource specifier "*"
  },
  {
    "effect": "deny",
    "actions": [
